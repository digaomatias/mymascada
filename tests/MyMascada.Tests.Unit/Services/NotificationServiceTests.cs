...
public class NotificationServiceTests
{
    ...
    [Fact]
    public async Task CreateNotification_PeriodDeduplicated_SkipsRateLimitAndCreatesDirectly()
    {
        // Period-deduplicated fan-out (budget alerts) must not be capped by the
        ...
        await _sut.CreateNotificationAsync(
            _userId, NotificationType.BudgetThreshold, "title", "body", groupKey: "gk-1", periodDeduplicated: true);
    ...
    [Fact]
    public async Task CreateNotification_PeriodDeduplicated_TreatsSoftDeletedAsExisting()
    {
        // A period-deduplicated alert the user already deleted must NOT be recreated
        // within the period, so the existence check includes soft-deleted rows.
        _preferenceRepo.GetByUserIdAsync(_userId, Arg.Any<CancellationToken>()).Returns((NotificationPreference?)null);
        // includeDeleted: true finds the soft-deleted prior alert.
        _notificationRepo.ExistsByGroupKeyAsync(_userId, "gk-1", true, Arg.Any<CancellationToken>()).Returns(true);

        await _sut.CreateNotificationAsync(
            _userId, NotificationType.BudgetThreshold, "title", "body", groupKey: "gk-1", periodDeduplicated: true);

        await _notificationRepo.DidNotReceive().CreateAsync(Arg.Any<Notification>(), Arg.Any<CancellationToken>());
        await _notificationRepo.DidNotReceive().CreateIfRateLimitNotExceededAsync(
            Arg.Any<Notification>(), Arg.Any<TimeSpan>(), Arg.Any<int>(), Arg.Any<CancellationToken>());
    }
    ...
    {
        _notificationRepo.ExistsByGroupKeyAsync(_userId, "gk-1", Arg.Any<bool>(), Arg.Any<CancellationToken>()).Returns(true);

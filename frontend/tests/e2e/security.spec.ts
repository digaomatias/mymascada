...
test.describe('Security & Authentication Edge Cases', () => {
  ...
  test('should prevent overdraft on insufficient funds', async ({ page }) => {
    ...
    const overdraftResponse = await page.evaluate(async ({ apiBaseUrl, data }) => {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`${apiBaseUrl}/transactions`, {
        method: 'POST',
        headers: {
    ...
    if (overdraftResponse.status === 200 || overdraftResponse.status === 201) {
      ...
      const accountCheck = await page.evaluate(async ({ apiBaseUrl, accountId }) => {
        const token = localStorage.getItem('auth_token');
  ...
  test('should prevent duplicate transaction submission', async ({ page }) => {
    ...
    const duplicateResponses = await page.evaluate(async ({ apiBaseUrl, data }) => {
      const token = localStorage.getItem('auth_token');
  ...
  test('should validate account deletion with existing transactions', async ({ page }) => {
    ...
    const deletionResponse = await page.evaluate(async ({ apiBaseUrl, accountId }) => {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`${apiBaseUrl}/accounts/${accountId}`, {
        method: 'DELETE',
        headers: {
    ...
    if (deletionResponse.status >= 400) {
      ...
      const transactionCheck = await page.evaluate(async (apiBaseUrl) => {
        const token = localStorage.getItem('auth_token');
        const response = await fetch(`${apiBaseUrl}/transactions`, {
          headers: {
            'Authorization': `Bearer ${token}`,

...
test.describe('CSV Preview - Reconciliation Screen', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Create a test account (fresh user has no accounts)
    await page.goto('/accounts');
    await page.click('button:has-text("Add Account")');
    await page.fill('#name', 'Test Checking');
    await page.selectOption('#type', '1'); // Checking account type enum value
    await page.fill('#currentBalance', '1000');
    await page.click('button:has-text("Create Account")');
    await expect(page.locator('text=Account created')).toBeVisible();
    
    // Navigate to reconciliation screen
    await page.click('table tbody tr:first-child'); // Click first account
    await page.click('button:has-text("Reconcile")');
    
    // Start reconciliation
    await page.fill('input[type="number"]', '1000'); // Statement ending balance
  ...
  test('should correctly preview negative-debits convention', async ({ page }) => {
    ...
    await testAmountConvention(page, 'negative-expense', [
  ...
  test('should correctly preview positive-debits convention', async ({ page }) => {
    ...
    await testAmountConvention(page, 'positive-expense', [
  ...
  test('should correctly preview unsigned_with_type convention', async ({ page }) => {
    ...
    // Select amount convention
    await page.selectOption('select[data-testid="amount-convention"]', 'type-column');
    
    // Map type values
    await page.click('button:has-text("Debit"):has-text("Expense")');
    await page.click('button:has-text("Credit"):has-text("Income")');
    
    await testAmountConvention(page, 'type-column', [
...
test.describe('CSV Preview - Cross-Screen Consistency', () => {
  test('should show identical results on both screens', async ({ page }) => {
    ...
    // Test reconciliation screen - create account first
    await page.goto('/accounts');
    await page.click('button:has-text("Add Account")');
    await page.fill('#name', 'Test Checking');
    await page.selectOption('#type', '1'); // Checking account type enum value
    ...
    await page.selectOption('select[data-testid="amount-convention"]', 'negative-expense');
    await page.click('button:has-text("Preview Data")');

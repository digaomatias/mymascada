  ...
  if (process.env.NODE_ENV === 'test' || process.env.PLAYWRIGHT_CLEAN_DATA === 'true') {
    ...
    try {
      ...
      if (loginResponse.ok()) {
        ...
        try {
          const rulesResponse = await page.request.get(`${API_BASE_URL}/rules`, {
            headers: {
          ...
          if (rulesResponse.ok()) {
            ...
            for (const rule of rules) {
              if (testRulePatterns.some(pattern => rule.name?.includes(pattern))) {
                ...
                await page.request.delete(`${API_BASE_URL}/rules/${rule.id}`, {
                  headers: {
                    'Authorization': `Bearer ${token}`
                  },
                  ignoreHTTPSErrors: true
        ...
        try {
          const transactionsResponse = await page.request.get(`${API_BASE_URL}/transactions`, {
            headers: {
              'Authorization': `Bearer ${token}`
            },
            ignoreHTTPSErrors: true
          ...
          if (transactionsResponse.ok()) {
            ...
            for (const transaction of transactions) {
              if (testTransactionPatterns.includes(transaction.description)) {
                ...
                await page.request.delete(`${API_BASE_URL}/transactions/${transaction.id}`, {
                  headers: {
                    'Authorization': `Bearer ${token}`
                  },
                  ignoreHTTPSErrors: true

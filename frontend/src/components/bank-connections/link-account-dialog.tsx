              ...
              {availableAkahuAccounts.length === 0 ? (
                ...
                availableAkahuAccounts.map((account) => (
                  <button
                    ...
                    className="w-full p-4 border border-ink-200 rounded-lg hover:border-blue-500 hover:bg-blue-50 transition-colors text-left"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium text-ink-900 truncate">{account.name}</h4>
                        <p className="text-sm text-ink-500">{account.bankName}</p>
                        <p className="text-xs text-ink-400 mt-0.5">{account.formattedAccount}</p>
                      </div>
              ...
              {loadingAccounts ? (
                <div className="space-y-3">
                  {Array.from({ length: 3 }).map((_, i) => (
                    <div key={i} className="animate-pulse p-4 bg-ink-100 rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-ink-300 rounded-lg" />
                        <div className="flex-1">
                          <div className="h-4 bg-ink-300 rounded w-1/3 mb-2" />
                          <div className="h-3 bg-ink-300 rounded w-1/4" />
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="text-center py-8">
                  <CurrencyDollarIcon className="w-12 h-12 text-ink-400 mx-auto mb-3" />
                  <h3 className="font-medium text-ink-900">{t('noAccountsAvailable')}</h3>
                  <p className="text-sm text-ink-500 mt-1">
                ...
                localAccounts.map((account) => (
                  <button
                    ...
                    disabled={isLinking}
                    className="w-full p-4 border border-ink-200 rounded-lg hover:border-green-500 hover:bg-green-50 transition-colors text-left disabled:opacity-50"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium text-ink-900 truncate">{account.name}</h4>
                        {account.institution && (
                          <p className="text-sm text-ink-500">{account.institution}</p>
                        )}
                      </div>
                      <div className="text-right">
                        <p className="font-medium text-ink-900">

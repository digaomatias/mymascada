        ...
        {step === 1 ? (
          ...
          >
            <div className="space-y-5">
              ...
              <label className="flex items-start gap-3 rounded-xl border border-ink-200 bg-primary-50/35 p-3">
                <Checkbox
                  checked={isRecurring}
                  onCheckedChange={(checked) => setIsRecurring(checked === true)}
                />
                <div>
                  <p className="text-sm font-medium text-ink-700">{t('wizard.isRecurring')}</p>
                  <p className="text-xs text-ink-500">{t('wizard.isRecurringHelp')}</p>
                </div>
              </label>
            </div>
          </BudgetWizardStepShell>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                {loadingCategories ? (
                  <div className="space-y-2">
                    {[1, 2, 3].map((item) => (
                      <div key={item} className="h-10 animate-pulse rounded-lg bg-primary-100/60" />
                    ))}
                  </div>
                ) : (
                  <div className="max-h-48 overflow-y-auto rounded-xl border border-ink-200">
              ...
              <div>
                ...
                {selectedCategories.length === 0 ? (
                  <p className="mt-2 rounded-xl border border-ink-200 bg-primary-50/35 p-3 text-sm text-ink-500">
                    {t('wizard.noCategoriesSelected')}
                  </p>
                ) : (
                  <div className="mt-3 space-y-3">
                    {selectedCategories.map((category) => (
                      <div key={category.categoryId} className="rounded-xl border border-ink-200 bg-white p-3">
            ...
            <div className="space-y-5">
              <section className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm font-semibold text-ink-900">{t(`wizard.${periodType.toLowerCase()}`)}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.1em] text-ink-500">
                    {t('wizard.dateRange')}
              ...
              <section className="rounded-xl border border-ink-200 bg-primary-50/35">
                <div className="border-b border-ink-200 px-4 py-3">
                  <p className="text-sm font-semibold text-ink-800">{t('wizard.categoriesAndAmounts')}</p>
                </div>
                ...
                <div className="flex items-center justify-between border-t border-ink-200 px-4 py-3">
                  <span className="text-sm font-semibold text-ink-700">{t('wizard.totalBudget')}</span>
                  <span className="text-lg font-semibold text-ink-900">{formatCurrency(totalBudget)}</span>
                </div>
              </section>

              <p className="text-xs text-ink-500">{t('wizard.nextAfterCreate')}</p>
            </div>
          </BudgetWizardStepShell>
        ) : null}
      </div>

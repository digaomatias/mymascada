...
export default function SettingsPage() {
  ...
  return (
    <AppLayout>
            ...
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {isLoadingCategories ? (
                    <p className="text-sm text-ink-500">{tCommon('loading')}</p>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed ${
                          aiCleaningEnabled ? 'bg-primary-600' : 'bg-ink-200'
                        }`}
                      >
                        <span
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${
                        dashboardTemplate === 'education' ? 'bg-primary-600' : 'bg-ink-200'
                      }`}
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
          ...
          {settingsItems.filter((item) => !item.featureFlag || features[item.featureFlag as keyof typeof features]).map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}>
                <Card className="rounded-[26px] border border-ink-200 bg-white/92 shadow-[0_20px_46px_-30px_rgba(47,129,112,0.20)] backdrop-blur-xs hover:shadow-[0_24px_52px_-28px_rgba(47,129,112,0.25)] transition-shadow cursor-pointer h-full">
                  <CardContent className="p-6">
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 bg-gradient-to-br from-primary-500 to-primary-600 rounded-xl flex items-center justify-center shrink-0">
                      ...
                      <ChevronRightIcon className="w-5 h-5 text-ink-400 shrink-0" />
                    </div>
                  </CardContent>
                </Card>
              </Link>
        ...
        {/* Coming Soon Section */}
        <div className="mt-8">
          <h2 className="text-lg font-semibold text-ink-600 mb-4">{t('comingSoon.title')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              const IconComponent = item.icon;
              return (
                <Card key={item.labelKey} className="rounded-2xl bg-white/60 border border-ink-200/40">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3 opacity-50">
                      <div className="w-10 h-10 bg-ink-200 rounded-xl flex items-center justify-center">
                        <IconComponent className="w-5 h-5 text-ink-500" />
                      </div>
                      <div>
                        <h3 className="font-medium text-ink-600">{t(`comingSoon.items.${item.labelKey}.title`)}</h3>
                        <p className="text-xs text-ink-500">{t(`comingSoon.items.${item.labelKey}.description`)}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-ink-500">{t('about.frontendVersion')}</span>
                      <span className="font-mono text-ink-900">{process.env.NEXT_PUBLIC_APP_VERSION ?? '0.0.0'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-ink-500">{t('about.backendVersion')}</span>
                      <span className="font-mono text-ink-900">{backendVersion ?? t('about.unavailable')}</span>
                    </div>
                  </div>
                </div>
              </div>

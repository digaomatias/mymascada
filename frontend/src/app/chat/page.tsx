...
const chatComponents: Partial<Components> = {
  ...
  h1: ({ children }) => (
    <div className="text-xs font-semibold text-primary-700 mt-3 first:mt-0 mb-1 pb-0.5 border-b border-ink-100">
      {children}
    </div>
  ),
  h2: ({ children }) => (
...
export default function ChatPage() {
  ...
  if (chatConfigured === false) {
    return (
      <AppLayout mainClassName="relative z-10 flex-1 flex items-center justify-center p-4 pb-24 md:pb-4">
          <div className="max-w-md w-full rounded-[26px] border border-ink-200 bg-white/92 shadow-[0_20px_46px_-30px_rgba(47,129,112,0.20)] backdrop-blur-xs p-8 text-center">
  ...
  return (
    <AppLayout mainClassName="relative z-10 flex-1 flex flex-col max-w-4xl mx-auto w-full pb-24 md:pb-0" noBackground>
        ...
        {hasMessages && (
          <div className="flex items-center justify-between px-4 py-2 border-b border-ink-200 bg-white/90 backdrop-blur-xs">
            <div className="flex items-center gap-2">
              <SparklesIcon className="w-5 h-5 text-primary-600" />
              <h1 className="text-sm font-medium text-ink-700">{t('title')}</h1>
            </div>
            <Button
              variant="ghost"
              size="sm"
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {(['spending', 'categories', 'budgets', 'saving', 'summary'] as const).map((key) => (
                    <button
                      ...
                      disabled={sending}
                      className="px-4 py-2 text-sm bg-white border border-ink-200/60 rounded-full text-ink-700 hover:bg-primary-50 hover:border-primary-300 hover:text-primary-700 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    >
            ...
            <>
              ...
              {sending && (
                <div className="flex justify-start">
                  <div className="bg-white border border-ink-200 rounded-2xl px-4 py-3">
          ...
          <div className="flex items-end gap-2">
            <div className="flex-1 relative">
              <textarea
                ...
                disabled={sending}
                rows={1}
                className="w-full resize-none rounded-xl border border-ink-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed"
                style={{ maxHeight: '150px' }}
              />
              {input.length > MAX_CHARS * 0.9 && (
                <span
                  className={`absolute bottom-1 right-2 text-xs ${
                    input.length >= MAX_CHARS ? 'text-red-500' : 'text-ink-400'
                  }`}
                >
